...
# ==================== 需要行级过滤的标签 ====================
# 第一类：合规检查类 —— 过滤后标签带 "+不合格"/"+异常"
COMPLIANCE_INSPECTION_TAGS = {"监督", "抽检", "检查", "抽查", "双公示", "双随机", "年检"}
# 第二类：执法类 —— 过滤后标签带 "+处罚"
...
BUSINESS_ENTITY_KEYWORDS = ("企业", "公司", "厂", "店")
NON_ENTERPRISE_UNIT_KEYWORDS = (
    "机关",
    "局",
    "委",
    "办",
...
def extract_companies(series):
    ...
    for value in series.dropna().astype(str).str.strip().tolist():
        ...
        if not value or lowered == "nan" or len(value) < 4:
            continue
        if should_exclude_business_like_name(value):
            continue
        companies.append(value)
    return list(dict.fromkeys(companies))


def should_exclude_business_like_name(name):
    """排除工商企业类名称：如果单位名称包含"企业/公司/厂/店"等商业关键词，
    且不包含任何非企机构关键词（政府/事业单位/社会组织等），则认为是工商企业，应排除。
    我们想要的是非工商企业的信息。
    """
    ...
    if not normalized:
        return False
    # 如果名称中包含商业实体关键词
    if not any(keyword in normalized for keyword in BUSINESS_ENTITY_KEYWORDS):
        return False
    # 如果同时包含非企机构关键词，则不排除（说明是非企机构）
    # 如果不包含任何非企机构关键词，则排除（说明是纯工商企业）
    return not any(keyword in normalized for keyword in NON_ENTERPRISE_UNIT_KEYWORDS)
...
def is_compliant_value(value):
    ...
    if "未发现" in stripped:
        return True
    if (
        stripped.endswith("合格")
        and "不合格" not in stripped
        and "不予合格" not in stripped
        and "未合格" not in stripped
    ):
        return True
    return False
...
def build_feature_matrix(index_file, output_file, csv_folder):
    ...
    cols_order = ["更新日期", "单位名称", "一级分类"] + target_columns + ["数据源"]
...
def export_feature_matrix(df_final, output_file):
    try:
        ...
        if total_rows <= chunk_size:
            ...
            print(f"   包含 {total_rows} 家单位 x {len(df_for_excel.columns)} 列特征")
        else:
            ...
            for index in range(num_chunks):
                ...
                print(f"   ✅ {chunk_file}: {len(chunk_df)} 家单位 ({start_idx + 1}-{end_idx})")
